...
class TsWanPtRssi(unittest.TestCase):
    ...
    def setUp(self):
        ...
        basic_airlink.slog("Step: Checking if testbed is READY")
        ...
        # step: check Firefox 
        basic_airlink.slog("Please close Web Browser if required \n")
...
#         
        ...
        ret = self.cb_ins.device_up(self.se_ins, self.driver)
        if ret == False:
            self.cb_ins.stop_simulator()
            self.assertEqual(ret, True, "Test Aborted")
    ...
    def test_remote(self):
        ...
        ping = ping_airlink.PingAirlink()
        remote_ping = self.proxy.deliver(ping)
        if remote_ping.ping_test('172.16.1.4'):
        ...
        if ret == True:          
            ...
            basic_airlink.slog("End-to-end Connection: Failed")
